...
#search-contents .btn-subscription {
	outline: none;
}
...
<script type="text/channel" id="sample-channel-div">
	<div class="col-md-12 channel-detail">
		<div class="row">
			...
			<div class="col-md-6 body">
				<div class="row top">
					<div class="col-md-12">
						...
						<div class="row">
							<span><span class="member-count"></span> 명 구독</span> <strong>&#183;</strong> <span class="video-count"></span>
						</div>
					</div>
				</div>
				<div class="row mid">
			...
			<div class="col-md-3 footer">
				<button class="btn btn-lg btn-subscription"><span class="member-count"></span> 명 구독</button>
			</div>
		</div>				
	</div>
</script>
...
<script>
	$(function() {
		...
		// -- subscribe btn clicked start
		
		$searchContents.on('click', '.btn-subscription', function(){
			if(!requestable){
				AppManager.showAlert('알림','다른 요청이 진행중입니다 잠시후에 다시 시도하세요');
				return;
			}
			requestable = false;
			
			var $thisBtn = $(this);
			var $thisChannel = $(this).parents('.channel-detail');
			var ownerNo = $thisChannel.find('.owner-no').text();
			var memberCount = $thisChannel.find('.member-count:eq(0)').text();
			
			$.ajax({
				
				url : AppManager.data.subscriptions.subscribe + "?ownerNo=" + ownerNo,
				type : 'get',
				success : function(result){
					if(result.errorMessage){
						AppManager.showAlert('오류','로그인이 필요한 서비스 입니다');
						requestable = true;
						return false;
					}
					...
					$.each(result.subscriptionList, function(index, channel){
						if(result.ownerNo == ownerNo){
							isMemeber = true;
						}
					});
					
					if($thisBtn.hasClass('btn-danger')){
						$thisBtn.removeClass('btn-danger').addClass('btn-light');
						memberCount++;
						...
						$thisBtn.removeClass('btn-light').addClass('btn-danger');
						memberCount--;
					...
					$thisChannel.find('.member-count').text(memberCount);
				},
				error : function(err){
					AppManager.showAlert('오류', '요청 중 요류가 발생했습니다 관리자에게 문의하세요');
				},
				complete : function(){
					requestable = true;
				}
		...
		$searchForm.submit(function(event) {
			...
			if(currPage != 0){
				...
				var searchType = $('#search-type').find('span.selected-option').attr('data-item');
			...
			$.ajax({
				...
				success : function(result) {
					...
					$.each(result.channels, function(i, channel) {
						...
						$newChannelDiv.find('.member-count').text(Utils.toZero(channel.memberCount));
						$newChannelDiv.find('.video-count').text('동영상 ' + channel.videoCount + '개');
						
						
						
						var $btnSubscription = $newChannelDiv.find('.btn-subscription');
						
						if(channel.ownerNo == result.loginUserNo){
							$btnSubscription.remove();
						} else {
							$btnSubscription.find('.member-count').html(Utils.toZero(channel.memberCount));
							if(channel.subscribed == "true"){
								$btnSubscription.removeClass('btn-danger').addClass('btn-light');
							} else {
								$btnSubscription.removeClass('btn-light').addClass('btn-danger');
							}
						}
